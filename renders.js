...
function paddedSize(size, style) {
  const {
    padding = 0, paddingLeft, paddingRight, paddingTop, paddingBottom,
  } = style;
...
export default Object.freeze({
  ...
  heading: (node: HeadingNode, output: OutputFunction, state: RenderState, styles: RenderStyles) => textContentRenderer('heading', `heading${node.level}`)(node, output, state, styles),
  ...
  link: (node: LinkNode, output: OutputFunction, state: RenderState, styles: RenderStyles) => {
    ...
    return (
      <Text key={state.key} style={styles.link} onPress={onPress ? () => onPress(node.target) : null}>
        {typeof node.content === 'string' ? node.content : output(node.content, state)}
      </Text>
    );
  },
